...
from platewatch_vision.application.consensus import normalize_plate
from platewatch_vision.domain.models import OCRResult


class PaddleOCRRecognizer:
    def __init__(self, engine: Any | None = None) -> None:
        self._engine = engine or self._load_engine()
    ...
    def recognize(self, image: Any) -> OCRResult | None:
        ...
        for line in lines:
            ...
            text = normalize_plate(str(recognition[0]))
            if not text:
                continue
            try:
                ...
                continue
            candidates.append(OCRResult(text=text, confidence=confidence))
